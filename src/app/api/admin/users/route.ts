...
export async function GET() {
  ...
  if (profilerError) return NextResponse.json({ error: profilerError.message }, { status: 500 });
...
export async function POST(request: Request) {
  ...
  if (inviteError || !invited.user) {
    return NextResponse.json({ error: inviteError?.message ?? "Kunde inte skicka inbjudan." }, { status: 500 });
  }
  ...
  if (profilError) {
    return NextResponse.json({ error: `Inbjudan skickad, men profilen kunde inte skapas: ${profilError.message}` }, { status: 500 });
  }
